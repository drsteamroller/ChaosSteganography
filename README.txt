...
Currently, chaos_unstego.py doesn't reproduce the exact same message out. My reasoning for this is float value rounding errors, which is an inevitability. I made sure to limit the floats generated by the chaos function to 3 significant figures, and both programs pick the same coordinates, so the values are being manipulated somehow.
